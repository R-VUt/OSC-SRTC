...
def main_thread():
  ...
  while not Stop_Event.is_set():
    try:
      ...
      recognized = Recognizer.ListenAndRecognize(recognizer , source_lang,
                                                Stop_Event, Recognizer.getDevices().index(mic), is_ptt, PTT_End, mic_vad_thresold, mic_min_record_time)
      to_send_message = ""
      ...
      if recognized != "":
        ...
        if source_lang != target_lang:
          ...
          translated = Translator.Translate(translator, recognized, source_lang,
                                            target_lang)
        else:
...
def option_changed(*args):
  ...
  if not Recognizer.isLanguageSupported(selected_recognizer, source_lang):
    GUI.print_log("[Error] This recognizer "+selected_recognizer+" does not support " + source_lang + " language.")
    GUI.set_property_value("recognizer_option", Recognizer.getRegisteredRecognizers()[0])

  if not Translator.isLanguageSupported(selected_translator, source_lang):
    GUI.print_log("[Error] This translator does not support " + source_lang + " language.")
    GUI.set_property_value("translator_option", Translator.getRegisteredTranslators()[0])
  
  if not Translator.isLanguageSupported(selected_translator, target_lang):
    GUI.print_log("[Error] This translator does not support " + target_lang + " language.")
    GUI.set_property_value("translator_option", Translator.getRegisteredTranslators()[0])

  if target2_lang != "None" and not Translator.isLanguageSupported(selected_translator, target2_lang):
    GUI.print_log("[Error] This translator does not support " + target2_lang + " language.")
    GUI.set_property_value("translator_option", Translator.getRegisteredTranslators()[0])

...
class SRTC_GUI(CTk):
  ...
  def __init__(self):
    ...
    self._logo_image = CTkImage(light_image=Image.open(resource_path("resources/logo.jpg")), dark_image=Image.open(resource_path("resources/logo.jpg")), size=(150, 150))

    self._navigation_frame = CTkFrame(self, corner_radius=0)
    self._navigation_frame_label = CTkLabel(self._navigation_frame, image=self._logo_image,
                                                              compound="top", font=CTkFont(size=17, weight="bold"))
    ...
    self._mic_label = CTkLabel(self._settings_tab.tab("system"), text="Microphone", height=25)
    self._Device_Selection = CTkOptionMenu(self._settings_tab.tab("system"), width=200, command=self._option_changed, dynamic_resizing=False, height=25)
    self._speech_label = CTkLabel(self._settings_tab.tab("system"), text="Speech Recognition", height=25)
    self._Recognizer_Selection = CTkOptionMenu(self._settings_tab.tab("system"), width=200, command=self._option_changed, dynamic_resizing=False, height=25)
    self._translator_label = CTkLabel(self._settings_tab.tab("system"), text="Translator", height=25)
    self._Translator_Selection = CTkOptionMenu(self._settings_tab.tab("system"), width=200, command=self._option_changed, dynamic_resizing=False, height=25)
    self._source_label = CTkLabel(self._settings_tab.tab("output"), text="Source", height=25)
    self._Source_Selection = CTkOptionMenu(self._settings_tab.tab("output"), width=150, command=self._option_changed, dynamic_resizing=False, height=25)
    self._target_label = CTkLabel(self._settings_tab.tab("output"), text="Target", height=25)
    self._Target_Selection = CTkOptionMenu(self._settings_tab.tab("output"), width=150, command=self._option_changed, dynamic_resizing=False, height=25)
    self._target2_label = CTkLabel(self._settings_tab.tab("output"), text="Target2", height=25)
    self._Target2_Selection = CTkOptionMenu(self._settings_tab.tab("output"), width=150, command=self._option_changed, dynamic_resizing=False, height=25)
    self._Romaji_Mode = IntVar()
    self._Romaji_Mode.set(0)
    self._Romaji_Mode_Checkbox = CTkCheckBox(self._settings_tab.tab("output"), text="Romaji Mode", variable=self._Romaji_Mode, command=self._option_changed, height=25)
    ...
    self._log_textbox = CTkTextbox(self._home_frame, state="disabled", width=400, corner_radius=0)
